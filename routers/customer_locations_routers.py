...
from functions.customer_loactions_functions import all_customer_locations, create_customer_locations_y, \
    update_customer_loactions_y, one_customer_l
...
def get_customers(search: str = None, id: int = 0,
...
def update_customers_location(this_customer_location: Update_customer_location,
                              db: Session = Depends(database),
                              current_user: CreateUser = Depends(get_current_active_user)):
    role_verification(user=current_user)
